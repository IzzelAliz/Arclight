...
public class ApplicationBootstrap extends ForgeBootstrap {
    ...
    protected void start(String... args) throws Exception {
        ...
        var jar = SecureJar.from(it -> new SimpleJarMetadata("arclight.launch", "1.0", Set.of("io.izzel.arclight.boot.application"), List.of()), arclight.getPrimaryPath());
        boot.add(jar);
    ...
    public void moduleMain(String... args) throws Exception {
        ...
        secure.add(SecureJar.from(it -> new SimpleJarMetadata(arclight.name(), arclight.moduleDataProvider().descriptor().rawVersion().orElse("1.0"),
            arclight.getPackages().stream().filter(p -> !p.equals("io.izzel.arclight.boot.application")).collect(Collectors.toSet()), arclight.getProviders()), arclight.getPrimaryPath()));

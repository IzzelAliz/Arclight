...
abstract class UploadFilesTask extends DefaultTask {
    ...
    private void uploadOne(File file) {
        ...
        (new URL(String.format(OBJECTS, sha1)).openConnection() as HttpURLConnection).with {
            ...
            if (it.responseCode != 200) {
                def reason = new String(it.inputStream.bytes)
    ...
    private static void link(String path, Object payload) {
        (new URL(String.format(FILES, path)).openConnection() as HttpURLConnection).with {
            it.setRequestMethod("PUT")
            it.doOutput = true
